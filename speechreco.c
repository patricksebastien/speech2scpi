...
output_result(Recog *recog, void *dummy)
{
  ...
  for(r=recog->process_list;r;r=r->next) {
    ...
    for(n = 0; n < r->result.sentnum; n++) { /* for all sentences */
      ...
      printf("\n");
      printf("totalscore %5.3f", ts / (seqnum -2));
      printf("\n");
      /* AM and LM scores */
	...
	// SEND TO PYTHON VIA OSC
	if((ts / (seqnum - 2)) > 0.6) {
		char recoString[4000] = "";
		for(i=1;i<seqnum - 1;i++) {
			strcat(recoString, winfo->woutput[seq[i]]);
			strcat(recoString, "-");
		}
		//printf("%s", recoString);
		if (lo_send(t, "/reco", "s", recoString) == -1) {
			printf("OSC error %d: %s\n", lo_address_errno(t),
				   lo_address_errstr(t));
		}
	}
...
main(int argc, char *argv[])
{
  ...
  if (jconf == NULL) {		/* error */
    fprintf(stderr, "Try `-help' for more infoffffrmation.\n");
    return -1;
  }
